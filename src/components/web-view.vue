  ...
  methods: {
    ...
    autoZoom: function () {
      if (this.currentTab.selector) {
        const code = `var element = document.getElementById("` + this.currentTab.selector +
          `");
        ...
        this.webview.getWebContents().executeJavaScript(code, false).then((result) => {
          ...
          if (this.currentTab.autoZoom) {
            const webviewHeight = this.webview.getBoundingClientRect().height;
            const webviewWidth = this.webview.getBoundingClientRect().width;
